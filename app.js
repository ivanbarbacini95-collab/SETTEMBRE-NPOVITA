...
function renderEntryGate() {
  ...
  if (firstAccess) {
    ...
    setEntryOnboardingStatus('L’indirizzo rimane salvato solo su questo dispositivo.');
    return;
  }
...
function showEntryGate() {
  ...
  if (!state.wallets.length) {
    requestAnimationFrame(() => setTimeout(() => $('entryWalletInput')?.focus(), 90));
  }
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.39').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  $('entryWalletInput')?.addEventListener('input', () => {
...
// v15.39 — PWA, Home gate, skeleton loading, data freshness, Live View auto-hide controls, smart reward milestones and INJ Node symbol.
