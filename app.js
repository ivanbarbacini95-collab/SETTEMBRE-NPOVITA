...
const state = {
  ...
  pulseAutoScale: { key: '', address: '', price: 1, holdings: 1, reward: 1, lastRewards: 0 },
...
function renderPulseView() {
  ...
  if (!ensurePulseDayBaseline()) {
    ...
    setPulseMetric('pulseMetricHoldings', 0, 0, '—', state.address ? formatInj(currentTotalInj(), 4) : '—', 'vs primo rilevamento di oggi');
  ...
  const holdingsDelta = total - b.baseTotal;
  const holdingsPct = b.baseTotal > 0 ? ((total / b.baseTotal) - 1) * 100 : 0;
  ...
  updatePulseRewardScaleAfterClaim(state.rewards, dailyReward);
  const priceVisual = pulseAdaptiveVisual(pricePct, 2, 'price');
  const holdingsVisual = pulseAdaptiveVisual(holdingsPct, 0.25, 'holdings');
  const rewardVisual = pulseAdaptiveVisual(rewardScale, 1, 'reward');
  ...
  setPulseMetric(
    'pulseMetricHoldings',
    holdingsDelta,
    holdingsVisual.level,
    pulseSignedInj(holdingsDelta, 4),
    state.address ? formatInj(total, 4) : '—',
    `${pulseSignedPercent(holdingsPct)} · vs inizio rilevato oggi${pulseScaleSuffix(holdingsVisual.multiplier)}`
  );
  setPulseMetric(
    'pulseMetricReward',
    state.rewards,
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.53').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.53 — Pulse View uses Holdings (total INJ) instead of Net Worth so Price, Holdings and Reward show independent signals.
