...
function renderPulseView() {
  ...
  if (!ensurePulseDayBaseline()) {
    ...
    setPulseMetric('pulseMetricWorth', 0, 0, '—', '—', 'vs apertura giornata');
    const rewardLevel = state.rewards > 0 ? pulseVisualLevel(state.rewards, 1) : 0;
    setPulseMetric('pulseMetricReward', state.rewards, rewardLevel, pulseSignedInj(state.rewards, 4), formatInj(state.rewards, 4), 'maturate · reset al claim');
    return;
  }
  ...
  const worthPct = b.baseWorth > 0 ? ((worth / b.baseWorth) - 1) * 100 : 0;
  const worthDelta = worth - b.baseWorth;
  ...
  setPulseMetric(
    ...
    b.fallback ? 'vs riferimento 24H' : 'vs apertura giornata'
  );
  setPulseMetric(
    'pulseMetricWorth',
    worthDelta,
    ...
    `${pulseSignedPercent(worthPct)} · oggi`
  );
  setPulseMetric(
    'pulseMetricReward',
    state.rewards,
    ...
    dailyReward > 0 ? `maturate · ~${formatInj(dailyReward, 4)}/giorno` : 'maturate · reset al claim'
  );
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.51').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.51 — Pulse View follows the current day; rewards reflect the live accrued amount and reset naturally after claim.
