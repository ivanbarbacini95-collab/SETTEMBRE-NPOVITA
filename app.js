...
function goHome() {
  ...
  const dialog = $('focusDisplayDialog');
  if (dialog?.open) {
    // Prepara la Home sotto il layer fullscreen e lascia che Live View
    // si dissolva sopra di essa: nessun flash della Dashboard.
    showEntryGate();
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.59').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
