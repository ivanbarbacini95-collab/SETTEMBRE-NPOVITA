...
function enterLiveViewFromGate() {
  ...
  // Live View viene montata come overlay fixed PRIMA di togliere l'Entry Gate,
  // così la dashboard non viene mai esposta e non dipendiamo dal top-layer di Safari.
...
function setFocusDisplayValue(element, text, numericValue) {
  ...
  if (!$('focusDisplayDialog')?.hidden && !state.suppressEffects && visibleChange && numericChange) {
    const direction = next > previous ? 'up' : 'down';
...
function openFocusDisplay() {
  ...
  renderFocusAmbient();
  dialog.hidden = false;
  dialog.classList.remove('controls-hidden', 'is-closing');
...
function closeFocusDisplay({ revealDashboard = true, animate = false } = {}) {
  const dialog = $('focusDisplayDialog');
  if (!dialog || dialog.hidden || dialog.classList.contains('is-closing')) return;
  window.clearTimeout(state.focusControlsTimer);
  dialog.classList.remove('controls-hidden');
  ...
  const finish = () => {
    ...
    // When returning Home, keep the outgoing layer mounted at opacity 0 for
    // two compositor frames. This avoids the one-frame black flash that iOS
    // can paint when a full-screen fixed layer is detached immediately.
    if (!revealDashboard) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          dialog.hidden = true;
          ...
          requestAnimationFrame(() => {
            ...
            document.body.classList.remove('mode-returning');
...
function goHome() {
  ...
  if (dialog && !dialog.hidden) {
    // Prepara la Home completamente ferma sotto Live View e poi esegue
    // un vero crossfade: niente micro-animazioni della Home durante l'uscita.
    showEntryGate({ modeReturn: true });
    requestAnimationFrame(() => requestAnimationFrame(() => closeFocusDisplay({ revealDashboard: false, animate: true })));
    return;
  }
  if (!$('pulseViewDialog')?.hidden) {
    showEntryGate({ modeReturn: true });
    requestAnimationFrame(() => requestAnimationFrame(() => closePulseView({ revealDashboard: false, animate: true })));
    return;
  }
  showEntryGate();
}
...
function renderDataFreshness() {
  ...
  if ($('entryGate')?.classList.contains('visible')) renderEntryOverview();
  renderSystemStatus();
  if (!$('focusDisplayDialog')?.hidden && $('focusDisplayClock')) $('focusDisplayClock').textContent = new Intl.DateTimeFormat('it-IT', { hour: '2-digit', minute: '2-digit' }).format(new Date());
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.62').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  $('closeFocusDisplayButton')?.addEventListener('click', goHome);
  $('closePulseViewButton')?.addEventListener('click', goHome);
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && !$('focusDisplayDialog')?.hidden) goHome();
    if (event.key === 'Escape' && !$('pulseViewDialog')?.hidden) goHome();
  });
...
// v15.62 — True layered exit: Live View no longer uses the native dialog top-layer; Entry Gate stays physically underneath both immersive overlays during the crossfade.
