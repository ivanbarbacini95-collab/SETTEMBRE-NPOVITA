...
function closeFocusDisplay({ revealDashboard = true, animate = false } = {}) {
  ...
  const finish = () => {
    ...
    if (!revealDashboard) {
      // Home is already below this layer. Detach only after opacity reached zero,
      // then reactivate the existing selector on the following compositor frame.
      dialog.hidden = true;
...
function goHome() {
  ...
  if (dialog && !dialog.hidden) {
    // Home is already painted underneath. Do not render or reopen it here.
    const gate = $('entryGate');
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.66').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.66 — Home title restored; wallet rename stays inside each rail card; horizontal wallet swipes update the account preview live.
