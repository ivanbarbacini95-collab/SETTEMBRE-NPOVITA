...
function renderEntryGate() {
  ...
  if (!host || !hint || !liveButton) return;

  if (state.wallets.length && !state.wallets.some((wallet) => wallet.address === state.entrySelectedAddress)) {
  ...
  if (!state.wallets.length) {
    ...
    hint.textContent = 'Nessun wallet salvato';
    const empty = document.createElement('div');
    empty.className = 'entry-wallet-empty';
    empty.innerHTML = '<strong>Nessun account</strong><span>Entra nella Dashboard e usa la lente per aggiungere un indirizzo.</span>';
    host.appendChild(empty);
    liveButton.disabled = true;
    $('entryNote').textContent = 'Live View sarà disponibile dopo aver aggiunto un wallet.';
    return;
  }
