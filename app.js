...
function closeFocusDisplay({ revealDashboard = true } = {}) {
  const dialog = $('focusDisplayDialog');
  if (!dialog?.open) return;
  window.clearTimeout(state.focusControlsTimer);
  dialog.classList.remove('controls-hidden');
...
function goHome() {
  ...
  if (dialog?.open) {
    showEntryGate();
    requestAnimationFrame(() => closeFocusDisplay({ revealDashboard: false }));
    return;
  }
...
function pulseExtremesStorageKey() {
  return 'inj_monitor_pulse_extremes_v1';
}
...
function updatePulseExtreme(metricKey, value, { epsilon = 1e-9, reset = false } = {}) {
  ...
  // Once the live candle reverses, freeze a translucent ghost at the extreme it left behind.
  if (direction && row.direction && direction !== row.direction) {
    if (row.direction > 0) row.ghostHigh = number(row.high);
    else row.ghostLow = number(row.low);
  }

  let flash = '';
  const now = Date.now();
  const cooldown = 2400;
  if (n > number(row.high) + epsilon) {
    row.high = n;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.56').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.56 — Live View and Pulse View are entry-menu destinations only; exiting either mode returns to the entry gate, never directly to Dashboard.
