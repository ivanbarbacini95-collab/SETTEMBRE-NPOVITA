...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.60').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  $('focusDisplayDialog')?.addEventListener('cancel', (event) => { event.preventDefault(); goHome(); });
  $('focusDisplayDialog')?.addEventListener('click', (event) => { if (event.target === $('focusDisplayDialog')) goHome(); });
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && $('focusDisplayDialog')?.open) goHome();
...
// v15.60 — Crossfade return: entry gate is pre-rendered without entrance motion; Live/Pulse dissolve over 720ms before teardown.
