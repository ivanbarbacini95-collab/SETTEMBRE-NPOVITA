...
function renderWorthFrame(key, prefix) {
  const row = $(`wtf${prefix}Row`);
  const minEl = $(`wtf${prefix}Min`);
  const openEl = $(`wtf${prefix}Open`);
  const maxEl = $(`wtf${prefix}Max`);
  const changeEl = $(`wtf${prefix}Change`);
  const track = $(`wtf${prefix}Track`);
  if (!row || !minEl || !openEl || !maxEl || !changeEl || !track) return;

  const frame = state.marketFrames[key];
  const totalInj = currentTotalInj();
  const current = totalInj * state.price;
  const open = totalInj * number(frame?.open);
  const min = totalInj * number(frame?.min);
  const max = totalInj * number(frame?.max);

  if (!state.address || !(totalInj > 0) || !(current > 0) || !(open > 0) || !(min > 0) || !(max > 0)) {
    minEl.textContent = '—';
    openEl.textContent = '—';
    maxEl.textContent = '—';
    changeEl.textContent = '—';
    row.className = 'market-tf worth-tf neutral';
    track.style.setProperty('--tf-pos', '50%');
    track.style.setProperty('--tf-open-pos', '50%');
    track.style.setProperty('--tf-fill-start', '50%');
    track.style.setProperty('--tf-fill-width', '0%');
  ...
  const start = Math.min(50, currentPosition);
  const width = Math.abs(currentPosition - 50);
  minEl.textContent = money(min, 2);
  openEl.textContent = money(open, 2);
  maxEl.textContent = money(max, 2);
  changeEl.textContent = `${changePercent > 0 ? '+' : ''}${changePercent.toFixed(Math.abs(changePercent) >= 100 ? 1 : 2)}%`;
  row.className = `market-tf worth-tf ${changePercent > 0 ? 'positive' : changePercent < 0 ? 'negative' : 'neutral'}`;
  track.style.setProperty('--tf-pos', `${currentPosition.toFixed(3)}%`);
  track.style.setProperty('--tf-open-pos', '50%');
  track.style.setProperty('--tf-fill-start', `${start.toFixed(3)}%`);
  track.style.setProperty('--tf-fill-width', `${width.toFixed(3)}%`);
  track.title = `Min ${money(min, 2)} · Apertura ${money(open, 2)} · Ora ${money(current, 2)} · Max ${money(max, 2)}`;
}

function renderWorthTimeframes() {
  renderWorthFrame('d1', '1d');
  renderWorthFrame('m1', '1m');
  renderWorthFrame('y1', '1y');
  renderWorthFrame('all', 'All');
}
...
function setTimeframeCollapsed(kind, collapsed, persist = true) {
  const card = kind === 'market' ? $('marketCard') : $('worthCard');
  const toggle = kind === 'market' ? $('marketTimeframeToggle') : $('worthTimeframeToggle');
  const panel = kind === 'market' ? $('marketTimeframesPanel') : $('worthTimeframesPanel');
  ...
  if (persist) {
    try { localStorage.setItem(`inj_monitor_${kind}_timeframes_collapsed_v1`, next ? '1' : '0'); } catch (_) {}
  }
}
...
function toggleTimeframes(kind) {
  const card = kind === 'market' ? $('marketCard') : $('worthCard');
  if (!card) return;
  setTimeframeCollapsed(kind, !card.classList.contains('timeframes-collapsed'));
}
...
function bindEvents() {
  ...
  $('worthTimeframeToggle').addEventListener('click', () => toggleTimeframes('worth'));
...
async function init() {
  ...
  setTimeframeCollapsed('worth', timeframeCollapsedPreference('worth'), false);
  renderAll();
  renderWalletControls();
  loadEurRate();
  await loadMarket();
