...
// INJ Monitor v16.0 — mobile-first UI, data logic preserved from v15.2.
...
function updateHeaderFreshness() {
  ...
  if (!label) return;
  if (!(state.lastDataAt > 0)) {
    label.textContent = '—';
    return;
  }
  const seconds = Math.max(0, Math.floor((Date.now() - state.lastDataAt) / 1000));
  if (seconds < 2) label.textContent = 'ora';
  else if (seconds < 60) label.textContent = `${seconds}s fa`;
  else if (seconds < 3600) label.textContent = `${Math.floor(seconds / 60)}m fa`;
  else label.textContent = new Date(state.lastDataAt).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });
}
...
function setStatus(mode, _message) {
  const host = $('syncState');
  if (!host) return;
  const offline = mode === 'offline' || navigator.onLine === false;
  const online = mode === 'online' && !offline;
  host.textContent = offline ? 'OFFLINE' : online ? 'LIVE' : 'SYNC';
  host.className = `sync-state ${offline ? 'offline' : online ? 'online' : 'syncing'}`;
}
...
function targetAppliedFeedback() {
  const button = $('targetApplyButton');
  const card = $('targetCard');
  if (!button || !card) return;
  clearTimeout(targetAppliedFeedback.timer);
  button.textContent = '✓';
  button.classList.add('confirmed');
  card.classList.add('target-confirmed');
  targetAppliedFeedback.timer = setTimeout(() => {
    button.textContent = 'OK';
    button.classList.remove('confirmed');
    card.classList.remove('target-confirmed');
  }, 1150);
}
...
function bindEvents() {
  ...
  const applyTargetInput = ({ feedback = false } = {}) => {
    ...
    renderTarget();
    if (feedback) {
      toast(`Target aggiornato a ${formatInj(target, target % 1 ? 3 : 0)}`);
      targetAppliedFeedback();
    }
    return true;
  };

  $('targetApplyButton').addEventListener('click', () => {
    if (applyTargetInput({ feedback: true })) {
      $('targetInput').blur();
      window.setTimeout(() => $('targetDialog')?.close(), 180);
    }
  });
...
async function init() {
  ...
  setInterval(updateHeaderFreshness, 1_000);
