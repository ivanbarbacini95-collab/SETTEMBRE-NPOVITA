...
function nativeChartAxisLabels(range, bounds) {
  ...
  const end = new Date(bounds.end - 1);
  if (range === 'h1') {
    return [
      start.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' }),
      new Date(bounds.end).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })
    ];
  }
  if (range === 'd1') return ['00:00', '24:00'];
  if (range === 'w1') return ['Lun', 'Dom'];
  if (range === 'm1') {
    return [
...
function renderNativeChart() {
  ...
  state.nativeChartRenderFrame = requestAnimationFrame(() => {
    ...
    const pad = { l: width < 480 ? 62 : 66, r: 16, t: 20, b: 36 };
    const plotW = Math.max(1, width - pad.l - pad.r);
    const plotH = Math.max(1, height - pad.t - pad.b);
    const bounds = { start: state.nativeChart.start, end: state.nativeChart.end };
    const now = Date.now();
    ...
    let minPerf = Math.min(0, ...perfValues);
    let maxPerf = Math.max(0, ...perfValues);
    let span = maxPerf - minPerf;
    if (span < 0.4) {
      const center = (maxPerf + minPerf) / 2;
      minPerf = center - 0.2;
      maxPerf = center + 0.2;
      span = 0.4;
    }
    const margin = Math.max(0.08, span * 0.12);
    minPerf -= margin;
    maxPerf += margin;
    span = maxPerf - minPerf;

    const xFor = (t) => pad.l + Math.max(0, Math.min(1, (t - bounds.start) / Math.max(1, bounds.end - bounds.start))) * plotW;
    ...
    const [leftLabel, rightLabel] = nativeChartAxisLabels(state.nativeChartRange, bounds);
    ...
    const progressX = xFor(Math.min(now, bounds.end));

    svg.setAttribute('viewBox', `0 0 ${width} ${height}`);
    const positiveTintHeight = Math.max(0, zeroY - pad.t);
    const negativeTintHeight = Math.max(0, height - pad.b - zeroY);
    ...
    svg.innerHTML = `
      <defs>
        <linearGradient id="nativeChartPositiveBg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="var(--up)" stop-opacity=".075"/>
          <stop offset="100%" stop-color="var(--up)" stop-opacity="0"/>
        </linearGradient>
        <linearGradient id="nativeChartNegativeBg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="var(--down)" stop-opacity="0"/>
          <stop offset="100%" stop-color="var(--down)" stop-opacity=".065"/>
        </linearGradient>
      </defs>
      <rect class="native-chart-zone positive" x="${pad.l}" y="${pad.t}" width="${plotW}" height="${positiveTintHeight}" fill="url(#nativeChartPositiveBg)"/>
      <rect class="native-chart-zone negative" x="${pad.l}" y="${zeroY}" width="${plotW}" height="${negativeTintHeight}" fill="url(#nativeChartNegativeBg)"/>
      <g class="native-chart-grid">
        <line x1="${pad.l}" y1="${pad.t}" x2="${width - pad.r}" y2="${pad.t}"/>
        <line x1="${pad.l}" y1="${zeroY}" x2="${width - pad.r}" y2="${zeroY}" class="zero"/>
        <line x1="${pad.l}" y1="${height - pad.b}" x2="${width - pad.r}" y2="${height - pad.b}"/>
        <line x1="${progressX}" y1="${pad.t}" x2="${progressX}" y2="${height - pad.b}" class="progress"/>
      </g>
      ${segmentMarkup}
      <circle class="native-chart-current ${currentClass}" cx="${last.x}" cy="${last.y}" r="5"/>
      ${nativeChartSvgText(pad.l - 10, pad.t + 4, topLabel, 'end', 'axis-value positive-axis')}
      ${nativeChartSvgText(pad.l - 10, zeroY + 4, '0%', 'end', 'axis-zero')}
      ${nativeChartSvgText(pad.l - 10, height - pad.b + 4, bottomLabel, 'end', 'axis-value negative-axis')}
      ${nativeChartSvgText(pad.l, height - 11, leftLabel, 'start', 'axis-time')}
      ${nativeChartSvgText(width - pad.r, height - 11, rightLabel, 'end', 'axis-time')}
    `;
...
function bindEvents() {
  ...
  window.addEventListener('resize', syncChartViewport, { passive: true });
  window.addEventListener('orientationchange', () => { requestAnimationFrame(syncChartViewport); setTimeout(syncChartViewport, 180); }, { passive: true });
  window.visualViewport?.addEventListener('resize', syncChartViewport, { passive: true });
  window.visualViewport?.addEventListener('scroll', syncChartViewport, { passive: true });
