...
const state = {
  ...
  pulseAutoScale: { key: '', address: '', price: 1, worth: 1, reward: 1, lastRewards: 0 },
...
function renderPulseView() {
  ...
  const worthVisual = pulseAdaptiveVisual(worthPct, 2, 'worth');
  ...
  setPulseMetric(
    ...
    `${pulseSignedPercent(worthPct)} · oggi${pulseScaleSuffix(worthVisual.multiplier)}`
  );
  setPulseMetric(
    'pulseMetricReward',
    state.rewards,
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.52').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.52 — Pulse View auto-scales each candle so extremes remain visible; value labels stay inside the safe visual area.
