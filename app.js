...
function chartThemeSettings() {
  const theme = document.documentElement.dataset.theme;
  if (theme === 'light') {
    return {
      theme: 'light',
      backgroundColor: '#eef5ff',
      gridColor: 'rgba(17, 53, 91, 0.075)'
    };
  }
  if (theme === 'black') {
    return {
      ...
      gridColor: 'rgba(255, 255, 255, 0.050)'
    };
  }
  return {
    theme: 'dark',
    backgroundColor: '#020708',
    gridColor: 'rgba(126, 242, 212, 0.050)'
  };
}

function syncChartViewport() {
  // La preview e' inline e segue automaticamente il proprio contenitore CSS.
}

function renderChartControls() {
  document.querySelectorAll('[data-chart-style]').forEach((button) => {
    const active = button.dataset.chartStyle === state.chartStyle;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
  document.querySelectorAll('[data-chart-interval]').forEach((button) => {
    const active = !state.chartRange && button.dataset.chartInterval === state.chartInterval;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
  document.querySelectorAll('[data-chart-range]').forEach((button) => {
    const active = button.dataset.chartRange === state.chartRange;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
}
...
function renderTradingViewChart() {
  const host = $('tradingViewHost');
  if (!host) return;

  renderChartControls();
  if (!$('chartDialog')?.open) return;
  host.replaceChildren();
  ...
  link.href = 'https://www.tradingview.com/symbols/INJUSDT/?exchange=BINANCE';
  link.rel = 'noopener nofollow';
  link.target = '_blank';
  link.textContent = 'INJ/USDT chart';
  ...
  script.async = true;
  const appearance = chartThemeSettings();
  const effectiveInterval = state.chartRange === 'ALL' ? 'M' : state.chartInterval;
  const chartConfig = {
    autosize: true,
  ...
  if (state.chartRange === 'ALL') chartConfig.range = 'all';
...
function openChartDialog() {
  ...
  if (!dialog) return;
  if (!dialog.open) dialog.showModal();
  requestAnimationFrame(renderTradingViewChart);
}

function closeChartDialog() {
  const dialog = $('chartDialog');
  if (dialog?.open) dialog.close();
  $('tradingViewHost')?.replaceChildren();
}

function setChartStyle(style) {
  if (!['1', '2'].includes(style)) return;
  state.chartStyle = style;
  localStorage.setItem('inj_monitor_chart_style', style);
  renderTradingViewChart();
}

function setChartInterval(interval) {
  const allowed = ['60', '720', 'D', 'W', 'M'];
  if (!allowed.includes(interval)) return;
  state.chartInterval = interval;
  state.chartRange = '';
  localStorage.setItem('inj_monitor_chart_interval', interval);
  localStorage.removeItem('inj_monitor_chart_range');
  renderTradingViewChart();
}
...
function setChartRange(range) {
  ...
  localStorage.setItem('inj_monitor_chart_range', range);
  renderTradingViewChart();
}
...
function applyTheme(theme, notify = false) {
  ...
  if ($('chartDialog')?.open) requestAnimationFrame(renderTradingViewChart);
...
function bindEvents() {
  ...
  $('chartDialog')?.addEventListener('close', () => $('tradingViewHost')?.replaceChildren());
  $('chartDialog')?.addEventListener('click', (event) => { if (event.target === $('chartDialog')) closeChartDialog(); });
  document.querySelectorAll('[data-chart-style]').forEach((button) => {
    button.addEventListener('click', () => setChartStyle(button.dataset.chartStyle));
  });
  document.querySelectorAll('[data-chart-interval]').forEach((button) => {
  ...
  window.addEventListener('orientationchange', () => requestAnimationFrame(syncChartViewport), { passive: true });
...
async function init() {
  try {
    ...
    state.chartStyle = ['1', '2'].includes(localStorage.getItem('inj_monitor_chart_style')) ? localStorage.getItem('inj_monitor_chart_style') : '1';
    state.chartInterval = ['60', '720', 'D', 'W', 'M'].includes(localStorage.getItem('inj_monitor_chart_interval')) ? localStorage.getItem('inj_monitor_chart_interval') : '60';
    state.chartRange = localStorage.getItem('inj_monitor_chart_range') === 'ALL' ? 'ALL' : '';
    if ($('currencyValue')) $('currencyValue').textContent = state.currency === 'EUR' ? '€' : '$';
