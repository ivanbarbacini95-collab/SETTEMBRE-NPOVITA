...
function renderNativeChart() {
  ...
  state.nativeChartRenderFrame = requestAnimationFrame(() => {
    ...
    const pad = { l: width < 480 ? 42 : 52, r: 16, t: 18, b: 34 };
    ...
    const area = `${path} L${last.x.toFixed(1)},${zeroY.toFixed(1)} L${first.x.toFixed(1)},${zeroY.toFixed(1)} Z`;
    ...
    svg.innerHTML = `
      <defs>
        <linearGradient id="nativeChartArea" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="var(--native-chart-line)" stop-opacity=".22"/>
          <stop offset="100%" stop-color="var(--native-chart-line)" stop-opacity="0"/>
        </linearGradient>
      </defs>
      ...
      <path class="native-chart-area" d="${area}"/>
      <path class="native-chart-line" d="${path}"/>
      <circle class="native-chart-current ${currentClass}" cx="${last.x}" cy="${last.y}" r="4.5"/>
      ${nativeChartSvgText(pad.l - 8, pad.t + 4, topLabel, 'end', 'axis-value')}
      ${nativeChartSvgText(pad.l - 8, zeroY + 4, '0%', 'end', 'axis-zero')}
      ${nativeChartSvgText(pad.l - 8, height - pad.b + 4, bottomLabel, 'end', 'axis-value')}
