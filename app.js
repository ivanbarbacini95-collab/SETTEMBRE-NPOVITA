...
function closeFocusDisplay({ revealDashboard = true, animate = false } = {}) {
  ...
  const finish = () => {
    ...
    $('entryGate')?.classList.remove('mode-return');
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.61').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.61 — True layered exit: Live View no longer uses the native dialog top-layer; Entry Gate stays physically underneath both immersive overlays during the crossfade.
