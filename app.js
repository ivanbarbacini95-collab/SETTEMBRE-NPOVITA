...
function renderEntryGate() {
  ...
  if (!shell || !host || !hint || !liveButton) return;
...
function closeEntryGate({ revealDashboard = true } = {}) {
  ...
  window.setTimeout(() => {
    ...
    if (revealDashboard || $('focusDisplayDialog')?.open) {
...
function syncChartViewport() {
  ...
  if (!dialog) return;
  const viewport = window.visualViewport;
  // iOS/PWA can briefly report a reduced visualViewport while a modal is
  // entering the top layer. Never let that transient value shrink the chart.
  const layoutWidth = window.innerWidth || document.documentElement.clientWidth || 0;
  const layoutHeight = window.innerHeight || document.documentElement.clientHeight || 0;
  const width = Math.max(280, Math.round(Math.max(number(viewport?.width), number(layoutWidth))));
  const height = Math.max(260, Math.round(Math.max(number(viewport?.height), number(layoutHeight))));
  dialog.style.setProperty('--chart-viewport-width', `${width}px`);
  dialog.style.setProperty('--chart-viewport-height', `${height}px`);
  dialog.style.setProperty('--chart-viewport-top', '0px');
  dialog.style.setProperty('--chart-viewport-left', '0px');
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.40').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
