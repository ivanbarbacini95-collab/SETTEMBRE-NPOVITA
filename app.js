...
function renderWalletCards() {
  ...
  board.hidden = !state.wallets.length;
  host.replaceChildren();
  if (!state.wallets.length) return;

  const synced = state.wallets.filter((wallet) => state.walletSummaries[wallet.address]).length;
  $('accountBoardStatus').textContent = state.summariesLoading
    ? `${synced}/${state.wallets.length} sincronizzati`
    : `${synced}/${state.wallets.length} account aggiornati`;
  ...
  state.wallets.forEach((wallet) => {
    ...
    card.type = 'button';
    card.className = `wallet-summary-card ${active ? 'active' : ''} ${fresh ? 'synced' : row ? 'cached-card' : 'loading-card'}`.trim();
    card.title = `Apri ${wallet.label}`;
    card.setAttribute('aria-label', `Apri dettaglio ${wallet.label}`);
    card.setAttribute('aria-pressed', active ? 'true' : 'false');

    const top = document.createElement('div');
    top.className = 'wallet-card-top';
    const name = document.createElement('span');
    name.className = 'wallet-card-name';
    name.textContent = wallet.label;
    const live = document.createElement('span');
    live.className = 'wallet-card-live';
    live.innerHTML = '<i></i>';
    live.append(document.createTextNode(active ? 'APERTO' : fresh ? 'LIVE' : row ? 'CACHE' : 'SYNC'));
    top.append(name, live);

    const address = document.createElement('small');
    address.className = 'wallet-card-address';
    address.textContent = shortAddress(wallet.address);

    const total = document.createElement('strong');
    total.className = 'wallet-card-total private';
    total.textContent = row ? formatInj(row.total, 3) : '—';

    const worth = document.createElement('small');
    worth.className = 'wallet-card-worth private';
    worth.textContent = row && state.price > 0 ? money(row.total * state.price) : 'Sincronizzazione dati…';

    const metrics = document.createElement('div');
    metrics.className = 'wallet-card-metrics';
    [
      ['Staking', row ? `${number(row.staked).toLocaleString('it-IT', { maximumFractionDigits: 2 })}` : '—'],
      ['Reward', row ? `${number(row.rewards).toLocaleString('it-IT', { maximumFractionDigits: 4 })}` : '—'],
      ['Liquidi', row ? `${number(row.available).toLocaleString('it-IT', { maximumFractionDigits: 2 })}` : '—']
    ].forEach(([label, value]) => {
      ...
      valueEl.classList.add('private');
      cell.append(labelEl, valueEl);
      metrics.appendChild(cell);
    });

    const updated = document.createElement('small');
    updated.className = 'wallet-card-updated';
    updated.textContent = summaryAge(row?.updated);

    card.append(top, address, total, worth, metrics, updated);
    card.addEventListener('click', () => {
      if (!active && !state.loading) loadWallet(true, wallet.address);
    });
...
function renderAggregate() {
  ...
  if ($('accountBoardStatus')) {
    $('accountBoardStatus').textContent = state.summariesLoading
      ? `${rows.length}/${state.wallets.length} sincronizzati`
      : `${rows.length}/${state.wallets.length} account aggiornati`;
  }
}
