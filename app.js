...
async function loadWallet(showFeedback = true, requestedAddress = '', forceMovements = false) {
  ...
  try {
    ...
    await loadWalletMovements(address, { force: switchingWallet || forceMovements }).catch(() => {});
...
function appendHookReward(rows, response, messageIndex, { hash, timestamp, typeUrl, detail = '' }) {
  const amount = rewardAmountFromResponse(response, messageIndex);
  if (!(amount > 0)) return;
  rows.push(movementRow({
    hash, timestamp, index: messageIndex + 400, kind: 'reward', label: 'Reward accreditate',
    amount, detail: detail || 'Accredito automatico staking', direction: 'in', typeUrl
  }));
}
  ...
  messages.forEach((message, index) => {
    ...
    if (lower.endsWith('msgdelegate')) {
      ...
      if (delegator === address) {
        ...
        appendHookReward(rows, response, index, { hash, timestamp, typeUrl, detail: shortAddress(message?.validator_address || '') });
      }
      return;
    }
    ...
    if (lower.endsWith('msgbeginredelegate')) {
      ...
      if (delegator === address) {
        ...
        appendHookReward(rows, response, index, { hash, timestamp, typeUrl, detail: shortAddress(message?.validator_src_address || '') });
      }
      return;
    }
...
function indexerTimestamp(item) {
  const direct = String(item?.block_timestamp || item?.blockTimestamp || item?.timestamp || '').trim();
  if (direct && !/^\d+(?:\.\d+)?$/.test(direct) && !Number.isNaN(new Date(direct).getTime())) return new Date(direct).toISOString();
  let raw = number(item?.block_unix_timestamp || item?.blockUnixTimestamp || item?.timeStamp || item?.timestamp);
  if (!(raw > 0)) return '';
...
function indexerTypeText(item) {
  const raw = item?.tx_msg_types ?? item?.txMsgTypes ?? item?.tx_type ?? item?.txType ?? item?.type ?? '';
...
function indexerDirectInjAmount(item) {
  const asset = String(item?.txAsset ?? item?.tx_asset ?? item?.asset ?? '').trim().toLowerCase();
  if (asset && !['inj', 'uinj'].includes(asset)) return 0;
  let value = number(item?.value ?? item?.txValue ?? item?.tx_value ?? item?.amount);
  if (!(value > 0)) return 0;
  // Gli accountTxs storici espongono normalmente INJ in unità umane. Se il valore è chiaramente uINJ/wei, normalizzalo.
  if (asset === 'uinj' || value > 1e12) value /= INJ_DECIMALS;
  return value;
}

function indexerSemanticKind(item) {
  const type = indexerTypeText(item).toLowerCase().replace(/[\s._/-]+/g, '');
  if (type.includes('withdrawdelegatorreward') || type.includes('withdrawreward')) return 'reward';
  if (type.includes('beginredelegate') || type.includes('redelegate')) return 'stake';
  if (type.includes('undelegate') || type.includes('unbond')) return 'unstake';
  if (type.includes('delegate')) return 'stake';
  return '';
}

function indexerHash(item) {
  return String(item?.hash || item?.txhash || item?.tx_hash || item?.txHash || item?.transactionHash || '').trim();
}

async function fetchTxDetailByHash(hash) {
  if (!hash) return null;
  return lcd(`/cosmos/tx/v1beta1/txs/${encodeURIComponent(hash)}`);
}
...
function indexerMovementRows(item, address, txIndex = 0) {
  ...
  if (!rows.length) {
    const semanticKind = indexerSemanticKind(item);
    const directAmount = indexerDirectInjAmount(item);
    if (semanticKind === 'reward') {
      rows.push(movementRow({
        hash, timestamp, index: txIndex * 1000 + 990, kind: 'reward', label: 'Prelievo reward',
        amount: directAmount, detail: 'Reward staking', direction: 'in', typeUrl: typeText
      }));
    } else if (semanticKind === 'stake') {
      rows.push(movementRow({
        hash, timestamp, index: txIndex * 1000 + 991, kind: 'stake', label: typeLower.includes('redelegat') ? 'Ridelega staking' : 'Messa in staking',
        amount: directAmount, detail: 'Staking', direction: 'stake', typeUrl: typeText
      }));
    } else if (semanticKind === 'unstake') {
      rows.push(movementRow({
        hash, timestamp, index: txIndex * 1000 + 992, kind: 'unstake', label: 'Uscita dallo staking',
        amount: directAmount, detail: 'Staking', direction: 'unstake', typeUrl: typeText
      }));
    }
  }

  if (!rows.length) {
    const fallbackLabel = movementTypeLabel(typeText || item?.tx_type || item?.txType || 'Transazione');
    rows.push(movementRow({
...
function indexerBatchRows(data, address) {
  return indexerTxItems(data).flatMap((tx, index) => indexerMovementRows(tx, address, index));
}

function indexerPageLength(data) {
  return indexerTxItems(data).length;
}

function indexerTotal(data) {
  return Math.max(0, number(
    data?.paging?.total || data?.pagination?.total || data?.total || data?.txNums || data?.tx_nums ||
    data?.data?.paging?.total || data?.data?.pagination?.total || data?.data?.total || data?.data?.txNums || data?.data?.tx_nums
  ));
}

async function hydrateIndexerSemanticRows(items, address) {
  const special = items
    .map((item, index) => ({ item, index, kind: indexerSemanticKind(item) }))
    .filter((entry) => entry.kind && indexerHash(entry.item));
  if (!special.length) return [];

  const hydrated = await mapWithConcurrency(special, 6, async ({ item, index }) => {
    try {
      const detail = await fetchTxDetailByHash(indexerHash(item));
      const rows = extractMovements(detail?.tx || {}, detail?.tx_response || {}, address);
      return rows.map((row) => ({ ...row, index: index * 1000 + row.index }));
    } catch (_) {
      return [];
    }
  });
...
async function loadMovementPageIndexer(address, entry, page, limit) {
  const [data, recentChain] = await Promise.all([
    explorerAccountTxs(address, page, limit),
    page === 1 ? txSearch(`message.sender='${address}'`, 1, 50).catch(() => null) : Promise.resolve(null)
  ]);
  const items = indexerTxItems(data);
  const summaryRows = items.flatMap((tx, index) => indexerMovementRows(tx, address, index));
  const hydratedRows = await hydrateIndexerSemanticRows(items, address);
  const recentChainRows = recentChain ? movementBatchRows(recentChain, address) : [];

  // Chain REST e dettagli per hash hanno priorità sui riassunti dell'Indexer. Questo evita il ritardo dei claim appena confermati.
  const hydratedHashes = new Set([...hydratedRows, ...recentChainRows].map((row) => row.hash).filter(Boolean));
  const nextRows = [
    ...summaryRows.filter((row) => !(hydratedHashes.has(row.hash) && ['reward', 'stake', 'unstake'].includes(row.kind))),
    ...hydratedRows,
    ...recentChainRows
  ];
  ...
  entry.source = recentChain ? 'indexer-chain' : 'indexer';
}
...
function renderMovements() {
  ...
  if (source && entry) {
    source.textContent = entry.source === 'lcd' ? 'LCD FALLBACK' : entry.source === 'indexer-chain' ? 'INDEXER + CHAIN' : entry.source === 'indexer' ? 'INDEXER LIVE' : 'INDEXER';
    source.classList.toggle('complete', ['indexer', 'indexer-chain'].includes(entry.source) && !entry.error);
...
async function refreshAll(showFeedback = true) {
  await Promise.all([
    loadMarket(),
    state.address ? loadWallet(false, state.address, true) : Promise.resolve()
  ]);
