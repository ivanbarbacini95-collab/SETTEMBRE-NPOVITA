...
function renderTradingViewChart() {
  ...
  const effectiveInterval = state.chartRange === 'ALL' ? 'M' : state.chartRange === '1M' ? '60' : state.chartInterval;
  const chartConfig = {
    autosize: true,
  ...
  if (state.chartRange === '1M') chartConfig.range = '1M';
...
function setChartInterval(interval) {
  const allowed = ['1', '5', '15', '60', '240', 'D', 'W'];
...
function setChartRange(range) {
  if (!['1M', 'ALL'].includes(range)) return;
...
function bindEvents() {
  ...
  $('closeChartDialog').addEventListener('click', closeChartDialog);
  $('chartDialog').addEventListener('click', (event) => {
    if (event.target === $('chartDialog')) closeChartDialog();
  });
  document.querySelectorAll('[data-chart-style]').forEach((button) => {
...
async function init() {
  try {
    ...
    state.chartInterval = ['1', '5', '15', '60', '240', 'D', 'W'].includes(localStorage.getItem('inj_monitor_chart_interval')) ? localStorage.getItem('inj_monitor_chart_interval') : '60';
    state.chartRange = ['1M', 'ALL'].includes(localStorage.getItem('inj_monitor_chart_range')) ? localStorage.getItem('inj_monitor_chart_range') : '';
    if ($('currencyValue')) $('currencyValue').textContent = state.currency === 'EUR' ? '€' : '$';
