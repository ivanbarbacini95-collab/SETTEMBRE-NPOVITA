const CACHE_NAME = 'inj-node-v15.47-shell';
const APP_SHELL = [
  './',
  './index.html',
  './style.css?v=15.47',
  './app.js?v=15.47',
  './manifest.webmanifest?v=15.47',
  './icons/icon-192.png',
  './icons/icon-512.png',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key !== CACHE_NAME && key.startsWith('inj-node-')).map((key) => caches.delete(key)))).then(() => self.clients.claim()));
});
